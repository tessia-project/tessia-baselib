...
from tessia_baselib.guests.linux.storage.disk_fcp import DiskFcp
...
class StoragePool(object):
    ...
    """
    def __init__(self, disks, host_conn):
        """
        Constructor

        Args:
            disks (list): containing each disk's parameters dict
            host_conn (SshClient): object connected to host
        """
    ...
    def _create_disk(self, parameters):
        ...
        return disk_cls(parameters, self._host_conn)
    # _create_disk()
    ...
    def _mpath_start(self):
        ...
        """
        shell = self._host_conn.open_shell()
    ...
    def activate(self):
        ...
        # build the response containing the device path of each activated disk
        dev_paths = {}
        while True:
            ...
            for vol_id, task in list(tasks.items()):
                ...
                # task ended successfully: store ret value and remove from map
                if task['ret']['exc'] is None:
                    dev_paths[vol_id] = task['ret']['value']
                    tasks.pop(vol_id)
                    continue
